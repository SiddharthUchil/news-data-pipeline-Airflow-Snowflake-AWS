...
with dag:
	...
	bash_command='aws s3 mv {{ ti.xcom_pull("extract_news_info")}}  s3://new22333',
	)
	
	snowflake_create_table=SnowflakeOperator(
		task_id="snowflake_create_table",
		sql="""create  table if not exists helloparquet using template(select ARRAY_AGG(OBJECT_CONSTRUCT(*)) from TABLE(INFER_SCHEMA (LOCATION=>'@NEWS.PUBLIC.snow_simple',FILE_FORMAT=>'parquet_format')))
        """ ,
	...
	snowflake_copy=SnowflakeOperator(
		...
		sql="""copy into NEWS.PUBLIC.helloparquet from @NEWS.PUBLIC.snow_simple MATCH_BY_COLUMN_NAME=CASE_INSENSITIVE FILE_FORMAT=parquet_format
        """ ,
		snowflake_conn_id="snowflake_conn"
	)
